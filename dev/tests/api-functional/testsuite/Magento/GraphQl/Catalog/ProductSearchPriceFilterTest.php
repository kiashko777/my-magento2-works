...
/**
 * Products filtering by condition "FROM..TO" for "Price" attribute
 *
 * Preconditions:
 ...
 *   Send request:
 *     query test {
 *       products(search: "Products", filter: {price: {from: "0.01" to: "9.99"}}, sort: {price: ASC}) {
 *         items {
 *           name
 *         }
 *         total_count
 ...
 *       "items": [
 *         {
 *           "name": "Products 2 $0.01"
 *         },
 *         {
 *           "name": "Products 3 $5"
 *         },
 *         {
 *           "name": "Products 4 $9.99"
 *         }
 *       ],
 *       "total_count": 3
 *     }
...
class ProductSearchPriceFilterTest extends GraphQlAbstract
{
    ...
    public function testProductSearchPriceFilter($priceFilter, string $sort, array $items): void
    {
        ...
        $query = <<<QUERY
{
  products(search: "Products", filter: {price: {{$priceFilter}}}, sort: {{$sort}}) {
    items {
      name
      price {
        minimalPrice {
